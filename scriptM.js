...
function submitProcess(e) {
	...
	document.getElementById("info").innerText = 'Keep the page open while the files upload';

	xhr.upload.addEventListener('progress', function (event) {
		uploadedCount = event.loaded / (event.total / fileCount);
		const percentComplete = (uploadedCount / fileCount) * 100;
		document.getElementById("submit-button").innerText = (Math.floor(percentComplete) > 99 ? 99 : Math.floor(percentComplete)) + '%';
	});
	...
	xhr.addEventListener('load', function (event) {
		if (xhr.status === 200) {
			...
			document.getElementById("info").innerText = 'You may safely close or refresh the page';
		} else {
			// Upload failed
